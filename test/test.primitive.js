...
tape( 'the function returns `false` if provided a number object whose value is a composite number', function test( t ) {
	t.strictEqual( isComposite( new Number( 6.0 ) ), false, 'returns expected value' );
	t.strictEqual( isComposite( new Number( 8.0 ) ), false, 'returns expected value' );
	t.strictEqual( isComposite( new Number( 10.0 ) ), false, 'returns expected value' );
	t.end();
});
